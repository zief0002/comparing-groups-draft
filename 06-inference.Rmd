...
```{r fig-01, fig.cap='Kernel density plots for the distribution of the T-scaled delinquency measure conditioned on treatment group.', fig.width=6, fig.height=4, out.width="60%"}
# Create plot of KDE
...
```{r fig-02, echo=FALSE, fig.width=8, fig.height=4, fig.cap="LEFT: Graphical depiction of the control and treatment populations of delinqunecy scores under the null hypothesis. RIGHT: Oone possible graphical depiction of the control and treatment populations of delinqunecy scores under the alternative hypothesis."}
# One population
x1 = seq(from = -3, to = 3, by = 0.01)
...
p1 = data.frame(x1, y1) %>%
  ggplot(aes(x = x1, y = y1)) +
    ...
    ylab("Probability density") +
    annotate(geom = "text", x = 0, y = 0.25, label = "Treatment\n&\nControl")
...
# Control
x3 = seq(from = -1, to = 5, by = 0.01)
y3 = dnorm(x3, mean = 2, sd = 1)

p2 = data.frame(
  x = c(x2, x3), 
  y = c(y2, y3), 
  group = c(rep("Treatment", 601), rep("Control", 601))
  ) %>%
  ggplot(aes(x = x, y = y, group = group)) +
    ...
    scale_x_continuous(
      ...
      limits = c(-5.5, 5.5),
      breaks = c(-2, 2),
      labels = c(expression(mu[Treatment]), expression(mu[Control]))
      ) +
    ylab("Probability density") +
    annotate(geom = "text", x = -2, y = 0.25, label = "Treatment") +
    annotate(geom = "text", x = 2, y = 0.25, label = "Control") +
...
```{r fig-03, echo=FALSE, out.width='5in', fig.cap='Thought experiment for sampling a "control" and "treatment" group from the population under the null hypothesis to obtain the sampling distribution of the difference in means.'}
include_graphics("images/notes-06-thought-experiment-means.pdf")
```


Mathematical theory suggests that the standard deviation of the sampling distribution, referred  to as the *standard error*, is

$$
SE_{\hat\mu_{\mathrm{Treatment}} - \hat\mu_{\mathrm{Control}}} =  \sqrt{\frac{\sigma^2}{n_{\mathrm{Control}}} + \frac{\sigma^2}{n_{\mathrm{Treatment}}}}
$$

where $\sigma^2$ is the variance in the population assumed under the null hypothesis (which, it turns out, is also the variance in the population of all delinquency scores) and $n_{\mathrm{Control}}$ and $n_{\mathrm{Treatment}}$ are the sample sizes for the control and treatment groups, respectively. So in our example,

$$
SE_{\hat\mu_{\mathrm{Treatment}} - \hat\mu_{\mathrm{Control}}} =  \sqrt{\frac{\sigma^2}{41} + \frac{\sigma^2}{37}}
$$

The complication here is that we do not know the value to substitute in for $\sigma^2$, since we do not know the variance for the population of delinquency scores.

<br /><br />

### Estimating $\sigma^2$: The t-Distribution





Fortunately, we can compute this through either simulate or mathematically 
...
<!-- ```{r echo=FALSE} -->
<!-- mu_j = c(rep(3.27, 4), rep(-3.64, 4)) -->

<!-- as_sub = after_school %>% -->
<!--   filter(row_number() %in% c(1:4, 75:78)) %>% -->
<!--   select(Group = treatment, Y = delinq) %>% -->
<!--   mutate(mu = 51.62, mu_j = mu_j, epsilon = Y - mu - mu_j) -->

<!-- vert_ellip = data.frame(Group = "&#8942;", Y = "&#8942;", mu = "&#8942;", mu_j = "&#8942;", epsilon = "&#8942;") -->

<!-- rbind(as_sub[1:4, ], vert_ellip, as_sub[5:8, ]) %>% -->
<!--   kable( -->
<!--     format = "html", -->
<!--     escape = FALSE, -->
<!--     col.names = c("Group", "$Y_{ij}$", "$\\hat\\mu$", "$\\hat\\mu_j$", "$\\hat\\epsilon_{ij}$"), -->
<!--     align = c("c"), -->
<!--     caption = "Outcome Values and Parameter Estimates for Eight Cases", -->
